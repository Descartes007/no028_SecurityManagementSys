...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑站岗信息</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>站岗信息管理</span></li>
								<li class="breadcrumb-item active"><span>编辑站岗信息</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">站岗信息信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									<input id="updateId" name="id" type="hidden">
									<div class="form-group col-md-6">
										<label>岗点名称</label>
										<input name="gangdianmingcheng" id="gangdianmingcheng" name="gangdianmingcheng" class="form-control" placeholder="岗点名称">
									</div>
									<div class="form-group col-md-6">
										<label>岗点图片</label>
										<div><img id="gangdiantupianImg" src="" width="100" height="100">
										<div class="upload">选择文件<input name="file" type="file" id="gangdiantupianupload" class="form-control-file"></div>
										<input name="gangdiantupian" id="gangdiantupian-input" type="hidden"></div>
									</div>
									<div class="form-group col-md-6">
										<label>岗点地址</label>
										<input name="gangdiandizhi" id="gangdiandizhi" name="gangdiandizhi" class="form-control" placeholder="岗点地址">
									</div>
									<div class="form-group col-md-6">
										<label>工号</label>
									...
									<div class="form-group col-md-6">
										<label>备注信息</label>
										<input name="beizhuxinxi" id="beizhuxinxi" name="beizhuxinxi" class="form-control" placeholder="备注信息">
									</div>
	...
	<script>
		...
		function gonghaoSelect2(){
			http("option/anbaorenyuan/gonghao","GET",{
		...
		function gonghaoSelect2Change(){
			...
			http("follow/anbaorenyuan/gonghao","GET",{
				tableName: "anbaorenyuan",
				columnName: "gonghao",
				columnValue: colVal
			},(res)=>{
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "zhangangxinxi/info/" + id,
		...
		// 文件上传
		function upload(){
			$('#gangdiantupianupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {				
					document.getElementById('gangdiantupian-input').setAttribute('value',"upload/"+data.result.file);
					if(document.getElementById('gangdiantupianFileName') != null){
						document.getElementById('gangdiantupianFileName').innerHTML = "上传成功!";
					}				
					$("#gangdiantupianImg").attr("src",baseUrl+"upload/"+data.result.file);								
				}
			});
		}  
		//取消 
				...
				if(crossrefid && crossuserid) {
					...
					http("zhangangxinxi/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("zhangangxinxi/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("zhangangxinxi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function calculationSE(colName){
			//单列求和接口
			http("zhangangxinxi"+colName,"GET",{
		...
		function dataBind(){
			...
			$("#gangdianmingcheng").val(ruleForm.gangdianmingcheng);	
			$("#gangdiantupian-input").val(ruleForm.gangdiantupian);
			$("#gangdiandizhi").val(ruleForm.gangdiandizhi);	
			...
			$("#beizhuxinxi").val(ruleForm.beizhuxinxi);	
		}		
		//图片显示
		function showImg(){
			var gangdiantupianFileName = "\"" + ruleForm.gangdiantupian + "\"";
			$("#gangdiantupianImg").attr("src",baseUrl+ruleForm.gangdiantupian);
		}		
		//跨表
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.gangdianmingcheng != null && res.data.gangdianmingcheng != ''){

						$("#gangdianmingcheng").val(res.data.gangdianmingcheng);
						$("#gangdianmingcheng").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.gangdiantupian != null && res.data.gangdiantupian != ''){

						$("#gangdiantupianImg").attr("src",baseUrl+res.data.gangdiantupian);
						$("#gangdiantupian-input").val(res.data.gangdiantupian);
					}
					if(res.data != null && res.data != '' && res.data.gangdiandizhi != null && res.data.gangdiandizhi != ''){

						$("#gangdiandizhi").val(res.data.gangdiandizhi);
						$("#gangdiandizhi").attr('readonly','readonly');
					}
					...
					if(res.data != null && res.data != '' && res.data.beizhuxinxi != null && res.data.beizhuxinxi != ''){

						$("#beizhuxinxi").val(res.data.beizhuxinxi);
						$("#beizhuxinxi").attr('readonly','readonly');
					}
				}
			});  
            }
